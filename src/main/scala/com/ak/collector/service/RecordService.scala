...
import com.ak.collector.repository.RecordStore

class RecordService[F[_]: Sync](getRecordStore: User => RecordStore[F]) extends Http4sDsl[F] {
  ...
  val routes: AuthedRoutes[User, F] = AuthedRoutes.of[User, F] {
    case GET -> Root / UUIDVar(id) as user =>
      for {
        recordOpt <- getRecordStore(user).getById(id)
        response <- result(recordOpt)
      } yield response
    case GET -> Root / "byLink" :? LinkQueryParamMatcher(link) as user =>
      for {
        recordOpt <- getRecordStore(user).findByLink(link)
        response <- result(recordOpt)
      } yield response
    case GET -> Root / UUIDVar(groupId) / "data.csv" as user => {
      val repository = getRecordStore(user)
      for {
        headers <- repository.recordsHeadersByGroup(groupId)
        resp <- Ok(
          (Stream[F, String](headers.mkString("\t")) ++ repository
            .recordsByGroup(groupId)
            .map(record => {
              val customInputMap: Map[String, String] = record.customInputs
                .foldLeft(List.empty[(String, String)])((values, field) =>
                  (field.name -> field.value) :: values
                )
                .toMap
    ...
    case usrReq @ POST -> Root as user =>
      for {
        ...
        id <- getRecordStore(user).create(reqBody)
        response <- Ok(id.toString())
      } yield response
    case usrReq @ PUT -> Root / UUIDVar(id) as user =>
      for {
        reqBody <- usrReq.req.as[RecordRequest]
        recordOpt <- getRecordStore(user).update(id, reqBody)
